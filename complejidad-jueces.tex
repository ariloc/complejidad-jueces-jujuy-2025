...
// El menor número mayor o igual a x está en arr[hi]
    ...
    \begin{frame}{$\mathcal{O}(n)$ (otro ejemplo)}
        \begin{block}{Problema (\only<1>{En busca de la mayor diversión}\only<2>{En busca de la mayor diversión, \textbf{Certamen Escolar 2021}}\only<3->{En busca de la mayor diversión, \textbf<3,4>{omegaUp 13769\tikzmark{judgeou}}})}
            \uncover<5->{
            ...
            \end{tikzpicture}
        \end{block}
        \uncover<6->{Es decir, queremos obtener la mayor suma posible de $n-1$ elementos de un arreglo de $n$ enteros positivos.} \uncover<7->{Esto es equivalente a sumar todos los números y restar el de menor valor.} 

        \uncover<8->{El código debería ser sencillo, no?}
    \end{frame}
...
while (arr[j] >= arr[i])
    ...
    \begin{frame}[noframenumbering]
        ...
        \item \textbf{Límite de memoria} (por caso): \pause De forma análoga al time limit, existe el \textbf{memory limit} (a veces abreviado \textit{ML}) o \textit{límite de memoria}, que es la cantidad de memoria que el programa puede reservar durante la ejecución de cada caso de prueba. \pause En el problema: \pause
            ...
            % TODO: Mencionar que ML no suele ser tan importante que TL, IOI vs ICPC
        \end{enumerate}
    \end{frame}
    ...
    %TODO: Eliminar referencias a certamen escolar (pero agregar algo para mí para acordarme mencionarlo). Repasar el doc entero y LIMPIAR!
